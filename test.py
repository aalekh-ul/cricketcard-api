...
null = None;
json_data = open("sa_apidata3.json").read()
...
for item in json_obj:
	...
	try:
		...
		matches=item.get("data").get("batting").get("T20Is").get("Mat")
	...
	cursor.execute("insert into crick_tt(runs,strikrate,matches,avg,wickets,economy,hundreds) values(%s,%s,%s,%s,%s,%s,%s)",(runs,strikrate,matches,avg,wickets,economy,hundreds))
